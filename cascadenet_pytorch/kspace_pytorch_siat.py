...
class DataConsistencyInKspace(nn.Module):
    ...
    def perform(self, x, k0, mask, model_save_dir=False,save_last=False):
        ...
        # mask_expanded = mask.unsqueeze(-1)  # [batch, H, W, 1]
        ...
        print('perform-x_res-1-dtype:',x_res.dtype) #perform-x_res-1-dtype: torch.float32
        
        # x_res = torch.view_as_real(x_res)  # 将复数拆分为实部和虚部作为两个通道
        x_res = torch.abs(x_res)  # 获取幅值
        print('perform-x_res-2-shape:',x_res.shape) #perform-x_res-2-shape: torch.Size([1, 2, 256, 256, 30])
        print('perform-x_res-2-dtype:',x_res.dtype) #perform-x_res-2-dtype: torch.float3
        return x_res
