...
import cascadenet_pytorch.kspace_pytorch_siat as cl
...
class DnCn(nn.Module):
    ...
    def __init__(self, n_channels=2, nc=3, nd=3, **kwargs):
...
class StochasticDnCn(DnCn):
    ...
    def __init__(self, n_channels=2, nc=3, nd=3, p=None, **kwargs):
...
class DnCn3DDS(nn.Module):
    ...
    def __init__(self, n_channels=2, nc=3, nd=3, fr_d=None, clipped=False, mode='pytorch', **kwargs):
        """
...
class CRNNcell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, kernel_size):
        ...
        self.h2h = nn.Conv2d(hidden_size, hidden_size, kernel_size, padding=self.kernel_size // 2)
        self.ih2ih = nn.Conv2d(hidden_size, hidden_size, kernel_size, padding=self.kernel_size // 2)
        self.i2h.weight = torch.nn.Parameter(self.i2h.weight.to(torch.float16))
        self.i2h.bias = torch.nn.Parameter(self.i2h.bias.to(torch.float16))
        self.h2h.weight = torch.nn.Parameter(self.h2h.weight.to(torch.float16))
        self.h2h.bias = torch.nn.Parameter(self.h2h.bias.to(torch.float16))
        self.ih2ih.weight = torch.nn.Parameter(self.ih2ih.weight.to(torch.float16))
        self.ih2ih.bias = torch.nn.Parameter(self.ih2ih.bias.to(torch.float16))
        self.relu = nn.ReLU(inplace=True)
...
class CRNN_MRI(nn.Module):
    ...
    def forward(self, x, k, m, test=False, model_save_dir=False,save_last=False):
        ...
        for i in range(1,self.nc+1):
            ...
            x = net['t%d_out'%i]
            # print('CRNN_MRI-x-2:dtype',x.dtype) #CRNN_MRI-x-2:dtype torch.float32
            # print('CRNN_MRI-x-2:shape',x.shape) #CRNN_MRI-x-2:shape torch.Size([1, 2, 256, 256, 30])
